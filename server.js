...
function presenceSnapshot(row, radiusMeters, now = Date.now()) {
  const radius = Number(radiusMeters);
  const lastDistance =
    row.last_presence_distance_m == null
      ? null
      : Number(row.last_presence_distance_m);
  const lastAccuracy =
    row.last_presence_accuracy_m == null
      ? null
      : Number(row.last_presence_accuracy_m);
  const ageSeconds = elapsedSeconds(row.last_presence_at, now);
  const outsideSeconds = elapsedSeconds(row.outside_since, now);

  const base = {
    required: Boolean(row.presence_required),
    state: "legacy",
    label: "Cadastro anterior",
    detail: "Sem verificação periódica.",
    canCall: true,
    blockReason: null,
    lastSeenAt: row.last_presence_at || null,
    ageSeconds,
    outsideSince: row.outside_since || null,
    outsideSeconds,
    distanceMeters: lastDistance,
    accuracyMeters: lastAccuracy,
    radiusMeters: radius,
    intervalSeconds: PRESENCE_INTERVAL_SECONDS,
    staleSeconds: PRESENCE_STALE_SECONDS,
    outsideGraceSeconds: PRESENCE_OUTSIDE_GRACE_SECONDS,
    maxAccuracyMeters: PRESENCE_MAX_ACCURACY_METERS,
  };
  ...
  if (row.manual) {
    return {
      ...
      detail: "Presença confirmada pelo responsável.",
    };
  }
  ...
  if (!row.last_presence_at || lastDistance == null) {
    return {
      ...base,
      state: "pending",
      label: "Aguardando GPS",
      detail: "O motorista precisa abrir o aplicativo.",
      canCall: false,
      blockReason: "Ainda não há uma verificação recente da localização deste caminhão.",
    };
  }

  if (
    outsideSeconds != null &&
    outsideSeconds >= PRESENCE_OUTSIDE_GRACE_SECONDS
  ) {
    const minutes = Math.max(1, Math.floor(outsideSeconds / 60));

    return {
      ...base,
      state: "outside",
      label: "Fora da unidade",
      detail: `Fora do raio há ${minutes} min.`,
      canCall: false,
      blockReason: `Este caminhão está fora da área permitida há aproximadamente ${minutes} minuto(s).`,
    };
  }
  ...
  if (ageSeconds != null && ageSeconds > PRESENCE_STALE_SECONDS) {
    ...
    return {
      ...
      canCall: false,
      blockReason: `A localização deste caminhão não é atualizada há aproximadamente ${minutes} minuto(s).`,
    };
  }

  if (
    lastAccuracy != null &&
    lastAccuracy > PRESENCE_MAX_ACCURACY_METERS
  ) {
    return {
      ...base,
      state: "inaccurate",
      label: "GPS impreciso",
      detail: `Precisão aproximada: ${lastAccuracy} m.`,
      canCall: false,
      blockReason: `A precisão atual do GPS é de aproximadamente ${lastAccuracy} metros.`,
    };
  }
  ...
  if (lastDistance > radius) {
    const remaining = Math.max(
      0,
      PRESENCE_OUTSIDE_GRACE_SECONDS - (outsideSeconds || 0)
    );
    ...
    return {
      ...
      state: "outside_grace",
      label: "Fora do raio",
      detail: `Distância atual: ${lastDistance} m. Tolerância: ${Math.ceil(
        remaining / 60
      )} min.`,
      canCall: false,
      blockReason: `Este caminhão está fora do raio permitido. Distância aproximada: ${lastDistance} metros.`,
    };
  }
  ...
  return {
    ...
    detail: `Distância atual: ${lastDistance} m.`,
  };
}
...
async function initDatabase() {
  await pool.query(`
    ...
    ALTER TABLE queue_entries
      ADD COLUMN IF NOT EXISTS presence_required BOOLEAN NOT NULL DEFAULT FALSE;

    ALTER TABLE queue_entries
      ADD COLUMN IF NOT EXISTS last_presence_at TIMESTAMPTZ;

    ALTER TABLE queue_entries
      ADD COLUMN IF NOT EXISTS last_presence_latitude DOUBLE PRECISION;

    ALTER TABLE queue_entries
      ADD COLUMN IF NOT EXISTS last_presence_longitude DOUBLE PRECISION;
    ...
    CREATE INDEX IF NOT EXISTS ix_queue_presence
      ON queue_entries (presence_required, last_presence_at)
  ...
  if (existing.rowCount === 0) {
    const initialCode =
      cleanText(process.env.DAILY_CODE, 12) ||
    ...
    const initialRadius = Math.max(
      20,
      Number(process.env.RADIUS_METERS || 200)
    );
    const blockDevice =
      String(process.env.BLOCK_DEVICE || "true").toLowerCase() !== "false";
...
app.get("/api/health", async (_req, res) => {
  try {
    ...
    res.json({ ok: true, version: "15.0.0" });
  } catch (error) {
    console.error(error);
...
app.get("/api/public-config", async (_req, res, next) => {
  try {
    ...
    res.json({
      ...
      presenceIntervalSeconds: PRESENCE_INTERVAL_SECONDS,
      presenceStaleSeconds: PRESENCE_STALE_SECONDS,
      presenceOutsideGraceSeconds: PRESENCE_OUTSIDE_GRACE_SECONDS,
      presenceMaxAccuracyMeters: PRESENCE_MAX_ACCURACY_METERS,
    });
  } catch (error) {
    next(error);
  }
...
app.post("/api/queue", publicWriteLimiter, async (req, res, next) => {
  ...
  try {
    ...
    const accuracy = normalizeAccuracy(req.body.accuracy);
    const presenceVersion = Number(req.body.presenceVersion || 0);
    const presenceRequired = presenceVersion >= 15;
    ...
    if (!deviceId) {
      return res
        .status(400)
        .json({ error: "Não foi possível identificar o aparelho." });
    }
    if (
    ...
    const inserted = await client.query(
      ...
      VALUES (
        ...
        CASE WHEN $10 THEN $6 ELSE NULL END,
        CASE WHEN $10 THEN $7 ELSE NULL END,
        CASE WHEN $10 THEN $5 ELSE NULL END,
    ...
    await audit(
      "PUBLIC_QUEUE_CREATED",
      {
        ...
        plate,
        distanceM: distance,
        presenceMonitoring: presenceRequired,
        presenceVersion,
      },
      client
    );
    ...
    res.status(201).json({
      ...
      presenceRequired,
      presence: presenceSnapshot(inserted.rows[0], settings.radius_m),
    });
  } catch (error) {
    ...
    if (error.code === "23505") {
      return res
        .status(409)
        .json({ error: "Esta placa já está ativa na fila." });
    }
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const currentResult = await client.query(
        `SELECT q.*, s.latitude AS unit_latitude,
                s.longitude AS unit_longitude, s.radius_m
         FROM queue_entries q
         CROSS JOIN settings s
         WHERE q.id = $1 AND s.id = 1
         FOR UPDATE OF q`,
        [req.params.id]
      );
      ...
      if (!safeCompare(deviceId, current.device_id || "")) {
        await client.query("ROLLBACK");
        return res.status(403).json({
          error: "Este aparelho não pertence a este registro da fila.",
        });
      }
      ...
      if (current.status !== "aguardando") {
        await client.query("ROLLBACK");
        return res.status(409).json({
          error: "A verificação foi encerrada porque o caminhão não está mais aguardando.",
          active: false,
      ...
      if (!current.presence_required) {
        ...
        return res.json({
          ...
          presence: presenceSnapshot(current, current.radius_m),
        });
      }
      ...
      const distance = Math.round(
        distanceMeters(
          latitude,
          longitude,
          Number(current.unit_latitude),
          Number(current.unit_longitude)
        )
      );
      ...
      const updatedResult = await client.query(
        `UPDATE queue_entries
         SET last_presence_at = NOW(),
             ...
             outside_since = CASE
               WHEN $4 IS NOT NULL AND $4 > $5 THEN outside_since
               WHEN $3 > $6 THEN COALESCE(outside_since, NOW())
               ELSE NULL
             END
         WHERE id = $7
         RETURNING *`,
      ...
      if (!current.outside_since && updated.outside_since) {
        await audit(
          "PRESENCE_OUTSIDE_STARTED",
          { id: updated.id, plate: updated.plate, distanceM: distance },
          client
        );
      } else if (current.outside_since && !updated.outside_since) {
        await audit(
          "PRESENCE_RETURNED",
          { id: updated.id, plate: updated.plate, distanceM: distance },
          client
      ...
      res.json({
        ...
        presence: presenceSnapshot(updated, current.radius_m),
      });
    } catch (error) {
      await client.query("ROLLBACK").catch(() => {});
      next(error);
...
app.get("/api/queue/:id/status", async (req, res, next) => {
  try {
    ...
    res.json({
      ...
      position,
      presenceRequired: Boolean(item.presence_required),
      presence: presenceSnapshot(item, item.radius_m),
    });
  } catch (error) {
    next(error);
  }
...
app.get("/api/admin/settings", requireAdmin, async (_req, res, next) => {
  try {
    const result = await pool.query(
      `SELECT unit_name, daily_code, latitude, longitude, radius_m,
              block_device, updated_at
    ...
    res.json({
      ...
      presence_stale_seconds: PRESENCE_STALE_SECONDS,
      presence_outside_grace_seconds: PRESENCE_OUTSIDE_GRACE_SECONDS,
      presence_max_accuracy_meters: PRESENCE_MAX_ACCURACY_METERS,
    });
  } catch (error) {
    next(error);
  }
...
app.put("/api/admin/settings", requireAdmin, async (req, res, next) => {
  try {
    ...
    if (unitName.length < 2) {
      return res.status(400).json({ error: "Nome da unidade inválido." });
    }
    if (dailyCode.length < 3) {
      return res.status(400).json({ error: "Código do dia inválido." });
    }
    ...
    if (!Number.isInteger(radiusM) || radiusM < 20 || radiusM > 10000) {
      return res
        .status(400)
        .json({ error: "O raio deve ficar entre 20 e 10.000 metros." });
    }
    ...
    const result = await pool.query(
      `UPDATE settings
       SET unit_name = $1,
           ...
           updated_at = NOW()
       WHERE id = 1
       RETURNING unit_name, daily_code, latitude, longitude,
                 radius_m, block_device, updated_at`,
...
app.get("/api/admin/queue", requireAdmin, async (_req, res, next) => {
  try {
    const settingsResult = await pool.query(
      `SELECT radius_m FROM settings WHERE id = 1`
    );
    const radius = settingsResult.rows[0].radius_m;

    const result = await pool.query(
      `SELECT id, plate, driver_name, carrier, status, arrival_time, updated_at,
              distance_m, latitude, longitude, manual, presence_required,
              last_presence_at, last_presence_latitude,
              last_presence_longitude, last_presence_distance_m,
              last_presence_accuracy_m, outside_since
       FROM queue_entries
       ORDER BY
    ...
    res.json(
      result.rows.map((row) => ({
        ...row,
        presence: presenceSnapshot(row, radius),
      }))
    );
  } catch (error) {
    next(error);
...
app.post("/api/admin/queue/manual", requireAdmin, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO queue_entries
        (id, plate, driver_name, carrier, manual, presence_required)
...
app.patch(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      const forcePresence = req.body.forcePresence === true;
      ...
      if (status === "chamado") {
        if (current.status !== "aguardando") {
          await client.query("ROLLBACK");
          return res.status(409).json({
            error: "Este motorista já foi chamado ou não está mais aguardando.",
          });
        }

        const presence = presenceSnapshot(current, current.radius_m);

        if (!presence.canCall && !forcePresence) {
          await client.query("ROLLBACK");
          return res.status(409).json({
            error: presence.blockReason,
      ...
      await audit(
        ...
        { id: req.params.id, status, forcePresence },
        client
      );
...
app.delete("/api/admin/queue", requireAdmin, async (req, res, next) => {
  try {
    ...
    const result =
      scope === "finished"
        ? await pool.query(
            `DELETE FROM queue_entries WHERE status = 'finalizado'`
          )
...
initDatabase()
  .then(() => {
    app.listen(PORT, "0.0.0.0", () => {
      console.log(`Fila de carregamento V15 iniciada na porta ${PORT}.`);
    });
  })
  .catch((error) => {
